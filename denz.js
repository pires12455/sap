...
prefix = "!"
name = "pires"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM USUÁRIO DO PIRES 😁"
...
memberLimit = 2
ban = []
premium = ["556996103915@s.whatsapp.net","6285724154826@s.whatsapp.net"]
...
ᴅɪɢɪᴛᴇ ${prefix}daftar ᴘᴀʀᴀ ᴄᴏᴍᴇᴄᴀʀ ᴀ ᴜsᴀʀ  ʙᴏᴛ`
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*⏳ ᴇsᴘᴇʀᴇ ᴛᴏᴍᴇ  ᴜᴍa água ᴇɴǫᴜᴀɴᴛᴏ ᴠᴏᴜ ᴛᴇ ᴇɴᴄᴀʀᴄᴀʀ ᴅᴇ ᴘᴇ...*',
				success: '*sᴜᴄᴇssᴏ...*',
				error: {
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
         ...
         case 'lirik':
                ...
                if (args.length < 1) return reply(`Mau request apa? Contoh: #request fitur anime`)
                    ...
                    denz.sendMessage('556996103915@s.whatsapp.net', options, text, {quoted: mek})
